...
static int osd_sem_creat(key_t key)
{
	union semun sem;
	int semid;
	sem.val = 0;
	semid = semget(key,1,IPC_CREAT|IPC_EXCL|0666);
	if(-1 == semid)//sem has been created.
	{
		printf("maybe sem has been created");
...
static  void osd_del_sem(INT32 semid)
{
	union semun sem;
	sem.val = 0;
	semctl(semid,0,IPC_RMID,sem);
}
static void osd_sem_wait(INT32 semid)
{
	struct sembuf sops={0,0,0};
	semop(semid,&sops,1);
}

static INT32 osd_sem_lock(INT32 semid)
{
...
static INT32 osd_sem_unlock(INT32 semid)
{
	struct sembuf sops={0,-1,IPC_NOWAIT};
	return (semop(semid,&sops,1));
}
...
static INT32 osdBlend(U16 x0,U16 y0,U16 x1,U16 y1)
{
	...
	ioctl(pFB->m_iGe2dFD, GE2D_CONFIG, &ge2d_config) ;
	...
	op_para.src2_rect.y= y0 + pFB->m_FBDevHeight * 2;
	...
	op_para.op = blendop( 
			...
			ALPHA_FACTOR_ONE,
			/* color blending dst factor */
			ALPHA_FACTOR_ZERO/*ALPHA_FACTOR_ONE_MINUS_SRC_ALPHA*/);
			
	ioctl(pFB->m_iGe2dFD, GE2D_BLEND, &op_para) ;
	return 0;
}
...
//printf("$$$osdUpdateRegion:%d,%d,%d,%d\n", x0, y0, x1, y1);
	...
	if (4 == pFB->m_FBDevColorDepth){		
		...
		ioctl(pFB->m_iGe2dFD, GE2D_BLIT_WITHOUTKEY, &op_para) ;
	...
	ioctl(pFB->m_iGe2dFD, GE2D_STRETCHBLIT, &op_para) ;

	osd_sem_unlock(g_osd_semid);
	return 0;
}
...
static INT32 FB_Enable32bitColorKey(T_FBAdaptor* pFB,INT32 layer, INT32 iColorKey,INT32 keymode)
{
	...
	ioctl( pFB->m_iGe2dFD, GE2D_SRCCOLORKEY, &config) ;	
	return 0;
}
//add by wjf end
...
INT32	FB_EraseRect(INT32 layer, INT32 nType, INT32 nLeft, INT32 nTop, INT32 nWidth, INT32 nHeight)
{
	...
	if(m_tMwDrvState.auColorKey[0] == MASKNONE)
	{
		...
		{//add by wjf
			op_para.color= 0x00;  //no transparent   BGRA
		}
		else
		{
			op_para.color=0x00;   
	...
	printf("color:0x%x\n",op_para.color);
	nResult = ioctl(pFB->m_iGe2dFD, GE2D_FILLRECTANGLE, &op_para) ;
...
printf("%s:%d\n",__FUNCTION__,__LINE__);
	...
	fb_var.yres_virtual = fb_var.yres * 4;       
	...
	*pMwFb = pFB->m_pucFrameBuffer + iFBOffset;
	...
	if (pFB->m_FBDevColorDepth != 4)
	{
		...
		printf("set color key mask none,0x%x\n",MASKNONE);
		m_tMwDrvState.auColorKey[0]	= MASKNONE;
	}
	key = ftok("/tmp",0);
	g_osd_semid=osd_sem_creat(key);	
