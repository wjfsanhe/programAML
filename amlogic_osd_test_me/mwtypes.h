...
#define MWTF_AUTO_NEWLINE	0x08000000L	/* auto newline*/
#define MWTF_HOZIRONTAL	0x10000000L	/* align on HOZIRONTAL EXTEND*/
#define MWTF_MIDDLE       	0x20000000L	/* align on MIDDLE*///luoxg add
...
#define MWTF_STRIKEOUT	0x0008		/* draw strikeout*/
...
/*图像平铺标志，可与其它操作同时存在*/
#define MWROP_TILE		0x80000000L	/* tile flag*/
...
//ZENGYI add all ops with alpha channel
/*图像、像素alpha通道 */
#define MWROP_ALPHA_CHANNEL	0x26000000L
/*标志最大扩展为0x7F000000L，因为最高位被图像平铺标志占用*/
...
typedef		struct
{
	unsigned char	bMode;	/* 透明度模式，取值及含义如下：*/
							/* 0-用于客户到服务器的传输，表示实际应该传空值 */
							/* 1-完全透明 */
							/* 2-完全不透明 */
							/* 3-统一的透明度 */
							/* 4-渐变透明度 */
							/* 0x40-清除所有叠加内容，此时其余参数无意义 */
							/* 0x80-0xFF-保留给应用程序使用 */
	unsigned char	bOption;			/* 模式选项，取值及含义与透明度模式有关 */
							/* 目前仅当渐变透明度时有意义，表示渐变模式： */
							/* 1-从左至右、2-从上到下、3-左上至右下、4-右上至左下 */
	unsigned char	bOrgTrans;		/* 初始透明度，当透明度模式为3、4时有意义 */
	unsigned char	bEndTrans;		/* 结束透明度，当透明度模式为4时有意义 */
	unsigned char	bColorFilter;	/* 色彩过滤，表示对特定颜色采取特殊的透明度设置： */
							/* 以表格来说明，"应用"表示使用前面字段的透明度设置 */
							/* 取值	特定颜色	其它颜色 */
							/* 0	应用		应用（其实是关闭色彩过滤） */
							/* 1	完全透明 	应用 */
							/* 2	完全不透明 	应用 */
							...
							/* 5	完全透明	完全不透明 */
							/* 6	应用		完全不透明 */
	unsigned int	uColorKey;		/* 用于色彩过滤的特定颜色即色键，用户API始终使用RGB24， */
							/* Nano-X服务器或驱动要将其映射成显示驱动支持的颜色 */
}MWTRANSPARENCE;
...
typedef struct {
	...
	long	lfStrikeOutColor;		/* color of font strike out add by lirihui */
...
#define	MWKMOD_REPEAT		0x0004		/* Key hold continuing */
...
typedef struct {
	int			state;
	unsigned int	retvlue;
}STATERETVALUE, *PSTATERETVALUE;
