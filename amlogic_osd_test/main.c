...
int main( int argc, char** argv)
{
	...
	MWCOORD    screenWidth;
        ...
        printf("please input antiflicker mode:");
        scanf("%d",&antiflicker);
        if(antiflicker)
        {
            MwEnableAntiflicker();
        }else{
            MwDisableAntiflicker();
        }
		...
		fclose(fp);
        	...
        	printf("#############%04x\n", color);
        	color = 0xFF00ff00; //ARGB 
        	unsigned int color2=MASKNONE;
        	unsigned char* p = (unsigned char*)pMwFb;
        	int i, count = 0, col = 0, j;
        	unsigned char* orgin = p;
		int high = 200, weight = 100;	

               p+=pnLineLen*200 + 500*4;    
		for(i = 0; i < high; i++){		
                	for (j = 0; j < weight; j++){       
                ...
                p=pMwFb+pnLineLen*250 + 520*4;  
                for(i = 0; i < high-200; i++){		
                	for (j = 0; j < weight-40; j++){       
                		memcpy(p + j * 4, &color2, 4);
                	}	          	          	
                	p += pnLineLen;	
                ...
                //read one picture onto FB canvas.
              p = (unsigned char*)pMwFb;
              FILE* fp = fopen("./pic", "rb");		
		char buffer[626 * 2 + 1];
               int *data; 
		fseek(fp, 0, SEEK_SET);	
		...
		for(i = 0; i < 446; i++){	
                      ...
                      for(j=0;j<626;j++)
                     {
                         fread(&color,sizeof(char),2,fp);
                         *data++=(0xff<<24)|((color>>11)&0x1f)<<18|((color>>5)&0x3f)<<10|color&0x1f;
                     }
			p += pnLineLen;			
	...
	ptRect.top = 0;
	ptRect.left = 0;
	ptRect.right = 700;//nScrW;//352;
	ptRect.bottom =600;// nScrH; //288;	
	printf("@@@@@@@@pic begin\n");
	...
	if (COLOR_TYPE == FB_COLORMODE_32){		
		...
		for(i = 0; i < high * weight; i++){		
                ...
                pnLineLen=weight*4;
                p=orgin+pnLineLen*30 +30*4;
                for(i = 0; i < high-60 ; i++){		
                	for (j = 0; j < weight-60; j++){       
                		memcpy(p + j * 4, &color, 4);
                	}	          	          	
                	p += pnLineLen;	
	...
	printf("@@@@@@@@pic begin,fsfsdafas\n");
 	MwFbRefreshBuf(&ptRect, buffer);
        ...
        //we will show one scroll subtitle.
        printf("start show scroll subtitle\n");
        i=0;
        
        while(1)
        {
            MwFbRefreshBuf(&ptRect, buffer2);
            ptRect.left --;
            ptRect.right--;
            if( ptRect.left ==10)
            {
