...
/*修改记录：CHENG SHENGYU 2006-02-13 添加 支持GIF动画*/
typedef  unsigned int   GR_GIFMAP_ID;    /* gifmap id */
/*修改结束：CHENG SHENGYU 2006-02-13*/
...
#define GR_WM_PROPS_DOUBLEWINDOW 0x00000080L /* double window */
...
/*******************************************
* 修改记录:CHENG SHENGYU 2006-04-06
* 保存图形上下文的line style属性
* 该结构被GrGetGCLinestyle函数使用
*******************************************/
typedef struct {
  GR_GC_ID       gcid;
  int                   linestyle;	
...
} GR_GC_LINESTYLE_INFO;
/*修改结束:CHENG SHENGYU 2006-04-06*/
...
/* expose tranparence interface struct to user */

typedef MWTRANSPARENCE  GR_TRANSPARENCE;
...
typedef enum 
{
	...
	GR_IMAGE_JPEG,
	GR_IMAGE_PNG,
	GR_IMAGE_BMP, 	// add by xch 2005-05-22
}GR_IMAGE_TYPE ;
...
/*typedef struct
{
    ...
    unsigned char sender[MAX_PROC_NAME];
} GR_EVENT_MSG;*/
...
void		GrDestroyRegionReserveID(GR_REGION_ID region);
...
void		GrSetFontLeftSpan(GR_FONT_ID fontid, GR_COORD leftspan);
...
void		GrSetDefaultFont(GR_FONT_ID fontid); // set default font,add by lirihui
...
/* 修改记录:CHENG SHENGYU 2006-03-22  获取GC的line style属性*/
void        GrGetGCLinestyle(GR_GC_ID gc, GR_GC_LINESTYLE_INFO *gclinestyle);
/*修改结束:CHENG SHENGYU 2006-03-22*/
...
* 函数名称：GrAreaEx
* 功能描述：从指定区域拷贝像素到屏幕窗口
* 输入参数：id-窗口标识，gc-窗口图形上下文，width-在窗口上显示的宽度，height-在窗口上显示的高度
*　　　　　sx-拷贝区域的左顶点x坐标，sy-拷贝区域的左顶点y坐标，scanline-行宽，pixels－像素缓冲区，从坐标(0,0)开始，
*                             pixtype－颜色模式
* 输出参数：无
* 返 回 值：无
* 备注：width<=scanline 
* 其它说明：为支持JAVA应用中客户向服务器频繁传输大数据量而增加
***************************************************************************/
void	        GrAreaEx(GR_DRAW_ID id, GR_GC_ID gc, GR_COORD x, GR_COORD y, GR_SIZE width, GR_SIZE height, 
						 GR_COORD sx, GR_COORD sy, GR_COORD scanline, void *pixels, int pixtype);

void            GrReqDataShmCmds(long datashmsize);
char            *GrGetDataShmMemAddr();
int             GrGetDataShmMemSize();
/*修改结束: CHENG SHENGYU 2006-01-13*/
...
void		GrDrawImageToFit(GR_DRAW_ID id, GR_GC_ID gc, GR_COORD x,
			GR_COORD y, GR_SIZE width, GR_SIZE height,
			GR_IMAGE_ID imageid ,int TransFlag, long trancl);
void		GrFreeImage(GR_IMAGE_ID id);
void		GrGetImageInfo(GR_IMAGE_ID id, GR_IMAGE_INFO *iip);
unsigned long		GrText(GR_DRAW_ID id, GR_GC_ID gc, GR_COORD x, GR_COORD y,
			void *str, GR_COUNT count, GR_TEXTFLAGS flags);
unsigned long GrGetTextSize(GR_SIZE nMaxWidth, GR_GC_ID gc, GR_COORD x, GR_COORD y, void *str,
...
/**************************************************************
修改记录:
*2005-4-25 10:00 钱静 增加，快速图像绘制接口函数声明
***************************************************************/
void        GrDrawImageNoClip(unsigned char *buf,
                              int X, int Y, int W, int fixH, int varH,
		                      int delta, int linedelta,
		                      GR_DRAW_ID psdID,
		                      unsigned int bg,int istrang,int sx,int sy);// add by xch 2005-5-20
/******************修改完毕************************************/
/**************************************************************
修改记录:
*2005-5-30 16:19 钱静 增加，告诉nano-X守护进程函数声明
***************************************************************/
void        GrInformDmn(void);
/******************修改完毕************************************/
void	GrBitBlt(GR_DRAW_ID, GR_GC_ID, GR_RECT*, GR_RECT*, GR_RECT*, void*, int);
unsigned long	GrRectRefresh(GR_RECT *ptRect, GR_CHAR_WIDTH bBlock);
unsigned long	GrRefreshRect(unsigned char bDevice, unsigned char bLayer, GR_RECT *ptRect, GR_CHAR_WIDTH bBlock);
unsigned long	GrWindowRefresh(GR_WINDOW_ID wid, GR_CHAR_WIDTH bBlock);
unsigned long	GrRegionRefresh(GR_REGION_ID rgnID, GR_CHAR_WIDTH bBlock);
void GrUnionWithClipRgn(GR_WINDOW_ID wid, GR_REGION_ID cliprgn, GR_REGION_ID rgn);
GR_WINDOW_ID GrConvertToPixmapFromFile(GR_CHAR *path, GR_SIZE *width, GR_SIZE *height, 
                                       GR_CHAR *transflag, GR_SIZE *transcolor,GR_IMAGE_TYPE type, GR_SIZE *channels, GR_SIZE *datasize);
GR_WINDOW_ID GrConvertToPixmapFromBuffer(GR_CHAR *imageData, GR_SIZE imageSize, GR_SIZE *width, 
                                         GR_SIZE *height, GR_CHAR *transflag, GR_SIZE *transcolor,
                                         GR_IMAGE_TYPE type, GR_SIZE *channels, GR_SIZE *datasize);
void GrGif_animator_play(GR_DRAW_ID wid, GR_GC_ID gc, GR_COORD x, GR_COORD y, GR_WINDOW_ID pixmapid);
void GrGif_animator_stop(GR_WINDOW_ID   pixmapid);
void GrGif_animator_resume(GR_DRAW_ID wid, GR_GC_ID gc, GR_COORD x, GR_COORD y, GR_WINDOW_ID pixmapid);

void GrDestroyGifmap(GR_WINDOW_ID id);
GR_GIFMAP_ID GrGetGifmapIDfromPixmapid(GR_WINDOW_ID   pixmapid);
void GrGetGifmapFrameInfo(GR_WINDOW_ID   pixmapid, unsigned char index, GR_WINDOW_ID *frameid, GR_SIZE *framew, GR_SIZE *frameh, GR_CHAR *transflag, GR_SIZE *transcolor);
unsigned char GrGetGifmapFrameNum(GR_WINDOW_ID   pixmapid);
void GrSetGCLineWidth(GR_GC_ID gc, unsigned short linewidth);
/*修改结束:CHENG SHENGYU 2006-02-13*/
void  GrAreaClip(GR_DRAW_ID id, GR_COORD x, GR_COORD y, GR_SIZE width,
	             GR_SIZE height, char *pixels, int pixtype);

int GrGetCoordinateMapPixel(GR_WINDOW_ID pixmapid, unsigned short x, unsigned short  y, unsigned int *pPixel);
int GrGetRGBMapPixel(unsigned char R, unsigned char G, unsigned char B, unsigned int *pPixel);
void GrSetImeInfo(IMEINFO * info);
void GrGetImeInfo(IMEINFO * info);
void GrShowWindow(GR_WINDOW_ID wid);
void GrSetResolution(GR_SIZE xres, GR_SIZE yres);

GR_GOVRECT_ID	GrCreateGovRect(unsigned char bDevice, unsigned char bLayer,
								GR_WINDOW_ID iWinId, GR_RECT *ptRect,
								GR_TRANSPARENCE *ptTran, unsigned char bReverse);
int		GrDeleteGovRect(GR_GOVRECT_ID iGovRect);
int		GrLayerControl(unsigned char bDevice, unsigned char bLayer,
			unsigned char bAction, unsigned char bArgB, unsigned int uArgU);
int		GrSetLayerTrans(unsigned char bDevice, unsigned char bLayer,
						unsigned char bAlpha, GR_COLOR	uColorKey);

int		GrNotifyVideoOn(unsigned char bDevice);
int		GrNotifyVideoOff(unsigned char bDevice, unsigned char bDelay);
int		GrQueryVideoState(unsigned char bDevice);

int		GrNotifySleepOn(unsigned char bDevice);
int		GrNotifySleepOff(unsigned char bDevice);
int		GrQuerySleepState(unsigned char bDevice);

int		GrSwitchGuiOn(unsigned char bDevice);
int		GrSwitchGuiOff(unsigned char bDevice);
int		GrQueryGuiState(unsigned char bDevice);
int		GrClearGui(unsigned char bDevice);

int		GrSwitchOsdOn(unsigned char bDevice);
int		GrSwitchOsdOff(unsigned char bDevice);
int		GrQueryOsdState(unsigned char bDevice);

/*add by qianj 2005/2/22 */

/**************************************************************************
* 函数名称：GrDiscardKeyEvent
* 功能描述：丢弃事件队列中的按键事件
* 输入参数：无
* 输出参数：无
* 返 回 值：无
* 其它说明：2006-10-19  罗贤国 增加
***************************************************************************/
void	GrDiscardKeyEvent();

/**************************************************************************
* 函数名称：GrStretchGif
* 功能描述：缩放GIF动画
* 输入参数：pixmapid -内存屏幕id，width-新宽度，height-新高度、
* 输出参数：无
* 返 回 值：缩放后的新GIF动画第一帧的 内存屏幕id
* 其它说明：2006-10-31  罗贤国 增加
***************************************************************************/
GR_DRAW_ID	GrStretchGif( GR_DRAW_ID pixmapid, int width,int height); 

/**************************************************************************
* 函数名称：GrSetGifFocusParam
* 功能描述：设置画焦点框函数的参数
* 输入参数：pixmapid -内存屏幕id，isfocus-是否需要画焦点框，rect -焦点框的矩形，linecolor-焦点框的颜色，linewidth-线的宽度
* 输出参数：无
* 返 回 值：无
* 备注：1、linecolor取值为GR_COLOR_RED等，具体参加文件nxcolors.h
*                  2、rect的左顶点坐标相对于当前焦点窗口。  
* 其它说明：2006-12-30  增加
***************************************************************************/
void	 GrSetGifFocusParam(GR_DRAW_ID pixmapid, GR_BOOL isfocus, GR_RECT *rect, GR_COLOR linecolor, GR_SIZE linewidth);

/**************************************************************************
* 函数名称：GrImageTransform
* 功能描述：首先图像翻转
* 输入参数：srcid           源图像的ID
                              width           宽度
                              height          高度
...
* 输出参数：无
* 返 回 值：翻转后的图像ID
* 其它说明：2007-01-08  斯彦维 增加
***************************************************************************/
GR_WINDOW_ID GrImageTransform(GR_WINDOW_ID srcid,  int x, int y, int width, int height, int channel,  int transform);
void	GrReadAreaARGB(GR_DRAW_ID id, GR_COORD x, GR_COORD y, GR_SIZE width,
			GR_SIZE height, void *pixels);
GR_WINDOW_ID
GrNewPixmapARGB(GR_COORD x, GR_COORD y, GR_SIZE width, GR_SIZE height, 
			void *pixels, int isAlpha);
/*void GrRegProcName(unsigned char * procname);
int GrSendMsg(char * sender,char * recver, unsigned char msgtype,
...
#define INIT_PER_THREAD_DATA()                                                  \
    {                                                                           \
        ecos_nanox_client_data *dptr = Z_MALLOC(sizeof(ecos_nanox_client_data));  \
