...
int main(int argc,char **argv)
{
	...
	if(argc<8)
	{
		printf("v_pid a_pid  s_pid v_format a_format freq font_path \n");
		printf("default value: 879 100 874 0 0  427000 /system/fonts/DroidSerif-Bold.ttf \n");
	}

	printf("Vpid[%d] Apid[%d] Tpid[%d] Vfmt[%d] Afmt[%d] Freq[%d] font[%s]\n", 
		v_pid, a_pid, s_pid, v_format, a_format, freq, path);
	...
	printf("open osd out[%dx%d]\n", para_osd.output_width, para_osd.output_height);
